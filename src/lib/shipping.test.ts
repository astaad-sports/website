...
import { parseOrderNumber } from "./format";
...
test("the timeline fills in as the order moves", () => {
  ...
  const steps = orderTimeline({ createdAt: placed, paidAt: paid, shippedAt: null, deliveredAt: null });
  expect(steps.map((step) => step.label)).toEqual(["Order placed", "Payment confirmed", "Shipped", "Delivered"]);
  expect(steps.map((step) => step.date !== null)).toEqual([true, true, false, false]);
});
