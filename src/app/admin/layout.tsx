...
export default async function AdminLayout({ children }: LayoutProps<"/admin">) {
  ...
  const [counts, newReviews] = await Promise.all([countOrdersByStatus(), countNewReviews()]);
  const toShip = (counts.paid ?? 0) + (counts.confirmed ?? 0) + (counts.packed ?? 0);

  return (
    <div className="flex flex-1 bg-surface text-foreground">
      <AdminSidebar toShip={toShip} newReviews={newReviews} name={user.name?.trim() || "Admin"} email={user.email} />
      <div className="flex min-w-0 flex-1 flex-col">
        <AdminHeader />
        <div className="flex flex-1 flex-col">{children}</div>
        <AdminTabBar toShip={toShip} newReviews={newReviews} />
      </div>
      <RefreshOnReturn />
    </div>
  );
