...
/**
 * An unknown or deleted review. Admins get a way back to Reviews; anyone else
 * (who reached here through requireAdmin's 404) sees a plain "not found"
 * that says nothing about the admin.
 */
  ...
  return (
    <main className={cn(PAGE, "pt-1 lg:pt-6")}>
      ...
      <div className="flex flex-col items-center gap-2 border-y border-border py-12 text-center">
        <MessageSquareQuote className="size-8 text-ink-muted" strokeWidth={1.5} aria-hidden="true" />
        <h1 className="text-base leading-[22px] font-semibold">This review doesn&apos;t exist</h1>
        <p className="text-[13px] leading-[18px] text-ink-muted">It may have been deleted. Find it in Reviews.</p>
        <Link href="/admin/reviews" className={cn(BUTTON_SECONDARY, "mt-2")}>
          Show all reviews
        </Link>
      </div>
    </main>
